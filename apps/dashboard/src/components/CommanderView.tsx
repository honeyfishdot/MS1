      ...
      <div className={`${styles.card} border-teal-500/15 bg-slate-900/30 backdrop-blur-md relative overflow-hidden`} id="commander-control-room">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-5">
          <div className={styles.innerCard} id="ctrl-profit-target">
            ...
            {settings.profitTargetAuto ? (<div className="py-2.5 px-3 bg-teal-950/10 border border-teal-500/10 rounded-lg flex items-center justify-between text-xs"><span className="text-teal-400 font-mono font-semibold flex items-center space-x-1"><Sparkles className="h-3 w-3 animate-spin shrink-0" /><span>AI Optimized</span></span><span className="font-mono font-bold text-teal-200">{formatCurrency(settings.profitTargetUsd)} USDC</span></div>) : (<div className="space-y-2"><div className="flex items-center justify-between text-xs font-mono text-slate-400"><span>Target Range:</span><span className="text-teal-300 font-bold">{formatCurrency(settings.profitTargetUsd)}</span></div><input type="range" min="100" max="2000" step="50" value={settings.profitTargetUsd} onChange={(e) => onUpdateSettings({ profitTargetUsd: parseFloat(e.target.value) })} className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-teal-500" /></div>)}
          ...
          <div className={styles.innerCard} id="ctrl-growth-scale">
            ...
            {settings.growthRateAuto ? (<div className="py-2.5 px-3 bg-amber-500/5 border border-amber-500/10 rounded-lg flex items-center justify-between text-xs"><span className="text-amber-400 font-mono font-semibold flex items-center space-x-1"><Sparkles className="h-3 w-3 animate-spin shrink-0" /><span>Copilot Model Active</span></span><span className="font-mono font-bold text-amber-200">{(settings.growthRate || 1.0).toFixed(1)}x leverage</span></div>) : (<div className="space-y-2"><div className="flex items-center justify-between text-xs font-mono text-slate-400"><span>Multiplier:</span><span className="text-amber-400 font-bold">{(settings.growthRate || 1.0).toFixed(1)}x</span></div><input type="range" min="0.5" max="5.0" step="0.1" value={settings.growthRate} onChange={(e) => onUpdateSettings({ growthRate: parseFloat(e.target.value) })} className="w-full h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-amber-500" /></div>)}
